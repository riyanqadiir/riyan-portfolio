...
/** Shared model type for order helpers (avoids Mongoose 9 + TS 5.9 Model<unknown> mismatch). */
type OrderableModel = Model<any>;

/** Push a new item to the top of the stack (order 0). Existing items shift down. */
export async function pushToTop(model: OrderableModel): Promise<number> {
...
export async function normalizeOrders(model: OrderableModel): Promise<void> {
